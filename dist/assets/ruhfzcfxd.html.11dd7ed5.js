import{_ as d}from"./_plugin-vue_export-helper.cdc0426e.js";import{o as a,c as s,a as i,d as e,b as r,e as l,r as t}from"./app.74ec0c3b.js";const c={},o={href:"https://mp.weixin.qq.com/s?__biz=MzkwODE5ODM0Ng==&mid=2247491980&idx=1&sn=22c357da998773d57115d71c3f5708c3&scene=21#wechat_redirect",target:"_blank",rel:"noopener noreferrer"},v=l(`<blockquote><p>关注公众号「<strong>三分恶</strong>」，回复「<strong>666</strong>」，领取七百多页独家原创的面试手册！</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/nice-article/weixin-ruhfzcfxd-2e3b7041-d317-4e79-9241-3dc098828a02.jpg" alt="" loading="lazy"></p><p>面渣逆袭手册</p></blockquote><h1 id="用户下单流程" tabindex="-1"><a class="header-anchor" href="#用户下单流程" aria-hidden="true">#</a> 用户下单流程</h1><p>我们从用户浏览商品开始，看看用户下单的简要过程：</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/nice-article/weixin-ruhfzcfxd-85e704f6-c73f-4881-8d30-ac80057f7bc3.jpg" alt="" loading="lazy"></p><p>用户下单简要过程</p><ul><li>浏览商品：用户查看商品详情</li><li>加购/结算：用户可以选择直接购买商品，也可以先加入购物车，用户购买的这一步就是结算</li><li>确认下单：结算完成，就进入了下单页面，<code>提交订单</code>，这一步就会生成一个订单，然后进入付款页面</li></ul><p>我们可以看到，下单是发生在结算之后，下单之后，会生成唯一的订单号，接下来，客户端需要用这个订单号去完成支付。</p><p>那接下来先看看，为什么发生重复下单？</p><h1 id="为什么会重复下单" tabindex="-1"><a class="header-anchor" href="#为什么会重复下单" aria-hidden="true">#</a> 为什么会重复下单</h1><p>为什么会重复下单，对于订单服务而言，就是接到了多个下单的请求，原因可能有很多，最常见的是这两种：</p><ul><li>用户重复提交</li><li>网络原因导致的超时重试</li></ul><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/nice-article/weixin-ruhfzcfxd-99b57ecc-6aa5-4100-ba56-63565c2dc037.jpg" alt="" loading="lazy"></p><p>重复下单原因</p><h1 id="如何防止重复下单" tabindex="-1"><a class="header-anchor" href="#如何防止重复下单" aria-hidden="true">#</a> 如何防止重复下单</h1><p>防止用户提交，最常规的做法，就是客户端点击下单之后，在收到服务端响应之前，按钮置灰。</p><p>当然，防止重复下单，肯定不能只依靠客户端，可能会因为一些网络的抖动，导致仍然有重复的请求到达服务端，所以还是要在服务端做防重/幂等的处理。</p><p>PS：这里额外插入一点我对防重和幂等的理解：防重指的是防止重复提交，幂等指的是多次请求如一次，简单说，就是防重可以给对重复请求抛异常，幂等是对重复的请求响应第一次的结果，在我们讨论的这个场景里，幂等就是响应唯一的订单号。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/nice-article/weixin-ruhfzcfxd-5a6a4a1d-66df-4fe7-80e9-fc6e2bd27bfd.jpg" alt="" loading="lazy"></p><p>防重和幂等</p><p>防重第一步，需要识别请求是否重复，这一步，需要客户端配合实现。</p><p>为什么呢？大家想一下，下单的时候，服务端怎么去判断这个下单请求是否唯一呢？金额？商品？优惠券？……万一用户就是喜欢，又下了一个一模一样的单呢？</p><p>所以，需要客户端在请求下单接口的时候，需要生成一个唯一的请求号：<code>requestId</code>，服务端拿这个请求号，判断是否重复请求。</p><p>那么，接下来，压力就给到服务端了，看看服务端怎么实现防重/幂等吧！</p><h2 id="利用数据库实现幂等" tabindex="-1"><a class="header-anchor" href="#利用数据库实现幂等" aria-hidden="true">#</a> 利用数据库实现幂等</h2><p>可以在订单表<code>t_order</code>里添加一个字段：<code>requestId</code>，添加唯一索引：</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/nice-article/weixin-ruhfzcfxd-0f6232ba-4e09-4ef8-a56a-076de147575f.jpg" alt="" loading="lazy"></p><p>唯一请求字段</p><p>这样一来，如果是重复的请求，在落库的时候就会报错，为了保证幂等性，我们可以catch住这个异常，根据requestId获取订单号，然后向客户端响应订单号。</p><p>大概的代码如下：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>PlaceOrderResVO placeOrder(PlaceOrderReqVO reqVO) {
  try {
    //下单业务逻辑
    ……
    //生成订单号
