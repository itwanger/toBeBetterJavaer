...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>RedisTemplate 和 StringRedisTemplate 都是 Spring Data Redis 提供的模板类，可以对 Redis 进行操作，后者针对键值对都是 String 类型的数据，前者可以是任何类型的对象。</p><p>RedisTemplate 和 StringRedisTemplate 除了提供 opsForValue 方法来操作字符串之外，还提供了以下方法：</p><ul><li>opsForList：操作 list</li><li>opsForSet：操作 set</li><li>opsForZSet：操作有序 set</li><li>opsForHash：操作 hash</li></ul><p>运行测试类后可以在控制台看到以下输出信息：</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-f4456aea-2e48-4bad-910d-2d963ef0224e.png" alt="" loading="lazy"></p><p>也可以通过 AnotherRedisDesktopManager 客户端查看 Redis 数据库中的数据。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-f7551ebb-0bde-4084-9ab0-4a724d8ad2ec.png" alt="" loading="lazy"></p><h2 id="编程喵整合-redis" tabindex="-1"><a class="header-anchor" href="#编程喵整合-redis" aria-hidden="true">#</a> 编程喵整合 Redis</h2><p>编程喵是一个 Spring Boot + Vue 的前后端分离项目，要整合 Redis 的话，最好的方式是使用 Spring Cache，仅仅通过 @Cacheable、@CachePut、@CacheEvict、@EnableCaching 等注解就可以轻松使用 Redis 做缓存了。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-eb6d69d7-9152-4695-87c7-cba138ca93fd.png" alt="" loading="lazy"></p><p><strong>1）@EnableCaching</strong>，开启缓存功能。</p><p><strong>2）@Cacheable</strong>，调用方法前，去缓存中找，找到就返回，找不到就执行方法，并将返回值放到缓存中。</p><p><strong>3）@CachePut</strong>，方法调用前不会去缓存中找，无论如何都会执行方法，执行完将返回值放到缓存中。</p><p><strong>4）@CacheEvict</strong>，清理缓存中的一个或多个记录。</p><p>Spring Cache 是 Spring 提供的一套完整的缓存解决方案，虽然它本身没有提供缓存的实现，但它提供的一整套接口、规范、配置、注解等，可以让我们无缝衔接 Redis、Ehcache 等缓存实现。</p><p>Spring Cache 的注解（前面提到的四个）会在调用方法之后，去缓存方法返回的最终结果；或者在方法调用之前拿缓存中的结果，当然还有删除缓存中的结果。</p><p>这些读写操作不用我们手动再去写代码实现了，直接交给 Spring Cache 来打理就 OK 了，是不是非常贴心？</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-afed274d-458d-4e6e-9fd0-b421ac811f47.png" alt="" loading="lazy"></p><p><strong>第一步</strong>，在 pom.xml 文件中追加 Redis 的 starter。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;dependency&gt;
    &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;
    &lt;artifactId&gt;spring-boot-starter-data-redis&lt;/artifactId&gt;
&lt;/dependency&gt;
...
<span class="token punctuation">}</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul><li>value：缓存名称，也就是缓存的命名空间，value 这里应该换成 namespace 更好一点；</li><li>key：用于在命名空间中缓存的 key 值，可以使用 SpEL 表达式，比如说 <code>&#39;codingmore:postags:&#39;+#postAddTagParam.postTagId</code>；</li><li>还有两个属性 unless 和 condition 暂时没用到，分别表示条件符合则不缓存，条件符合则缓存。</li></ul><p><strong>第五步</strong>，启动服务器端，启动客户端，修改标签进行测试。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-6463fdda-6cc2-43f4-91e6-e0de9f0f1c0c.png" alt="" loading="lazy"></p><p>通过 Red 客户端（一款 macOS 版的 Redis 桌面工具），可以看到刚刚更新的返回值已经添加到 Redis 中了。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-468a253d-931a-4e5b-8f7e-36ecc1561dac.png" alt="" loading="lazy"></p><h2 id="使用-redis-连接池" tabindex="-1"><a class="header-anchor" href="#使用-redis-连接池" aria-hidden="true">#</a> 使用 Redis 连接池</h2><p>Redis 是基于内存的数据库，本来是为了提高程序性能的，但如果不使用 Redis 连接池的话，建立连接、断开连接就需要消耗大量的时间。</p><p>用了连接池，就可以实现在客户端建立多个连接，需要的时候从连接池拿，用完了再放回去，这样就节省了连接建立、断开的时间。</p><p>要使用连接池，我们得先了解 Redis 的客户端，常用的有两种：Jedis 和 Lettuce。</p><ul><li>Jedis：Spring Boot 1.5.x 版本时默认的 Redis 客户端，实现上是直接连接 Redis Server，如果在多线程环境下是非线程安全的，这时候要使用连接池为每个 jedis 实例增加物理连接；</li><li>Lettuce：Spring Boot 2.x 版本后默认的 Redis 客户端，基于 Netty 实现，连接实例可以在多个线程间并发访问，一个连接实例不够的情况下也可以按需要增加连接实例。</li></ul><p>它俩在 GitHub 上都挺受欢迎的，大家可以按需选用。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-c94651b5-1e53-42e5-ad5f-162b4bf509a7.png" alt="" loading="lazy"></p><p>我这里把两种客户端的情况都演示一下，方便小伙伴们参考。</p><p><strong>1）Lettuce</strong></p><p><strong>第一步</strong>，修改 application-dev.yml，添加 Lettuce 连接池配置（pool 节点）。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>spring:
    redis:
        lettuce:
          pool:
    ...
    &lt;scope&gt;compile&lt;/scope&gt;
&lt;/dependency&gt;
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>重新启动服务，在 RedisConfig 类的 redisTemplate 方法里对 redisTemplate 打上断点，debug 模式下可以看到连接池的配置信息（<code>redisConnectionFactory→clientConfiguration→poolConfig</code>）。如下图所示。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-e4cd346c-07d0-4ee3-9832-4c7a2aa1b7b4.png" alt="" loading="lazy"></p><p>如果在 application-dev.yml 文件中没有添加 Lettuce 连接池配置的话，是不会看到</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-7e86e208-bf5f-4dc2-a962-2b926adaa524.png" alt="" loading="lazy"></p><p><strong>2）Jedis</strong></p><p><strong>第一步</strong>，在 pom.xml 文件中添加 Jedis 依赖，去除 Lettuce 默认依赖。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&lt;dependency&gt;
    &lt;groupId&gt;org.springframework.boot&lt;/groupId&gt;
    &lt;artifactId&gt;spring-boot-starter-data-redis&lt;/artifactId&gt;
    &lt;exclusions&gt;
        &lt;exclusion&gt;
    ...
    redis:
        jedis:
          pool:
            ...
            min-idle: 0 # 连接池最小空闲连接数
            max-wait: -1ms # 连接池最大阻塞等待时间，负值表示没有限制
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>启动服务后，观察 redisTemplate 的 clientConfiguration 节点，可以看到它的值已经变成 DefaultJedisClientConfiguration 对象了。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-01aa7dc6-b9f7-46bd-b8a4-0a24e44185bc.png" alt="" loading="lazy"></p><p>当然了，也可以不配置 Jedis 客户端的连接池，走默认的连接池配置。因为 Jedis 客户端默认增加了连接池的依赖包，在 pom.xml 文件中点开 Jedis 客户端依赖可以查看到。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-c87d8e02-aace-4d86-8011-13e4087956e0.png" alt="" loading="lazy"></p><h2 id="自由操作-redis" tabindex="-1"><a class="header-anchor" href="#自由操作-redis" aria-hidden="true">#</a> 自由操作 Redis</h2><p>Spring Cache 虽然提供了操作 Redis 的便捷方法，比如我们前面演示的 @CachePut 注解，但注解提供的操作非常有限，比如说它只能保存返回值到缓存中，而返回值并不一定是我们想要保存的结果。</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-f28a3b84-ed0b-4a78-a5e5-5803bae967be.png" alt="" loading="lazy"></p><p>与其保存这个返回给客户端的 JSON 信息，我们更想保存的是更新后的标签。那该怎么自由地操作 Redis 呢？</p><p><img src="https://cdn.tobebetterjavaer.com/tobebetterjavaer/images/redis/redis-springboot-9b89af51-c2fd-4b2d-ba57-a59efa4cbffd.png" alt="" loading="lazy"></p><p><strong>第一步</strong>，增加 RedisService 接口：</p><div class="language-java line-numbers-mode" data-ext="java"><pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">interface</span> <span class="token class-name">RedisService</span> <span class="token punctuation">{</span>
