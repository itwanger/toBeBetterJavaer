...
所以，[内存可见性](https://javabetter.cn/thread/thread-bring-some-problem.html)是针对的是**共享变量**。
...
Java 内存模型 (JMM) 主要针对的是多线程环境下，如何在主内存与工作内存之间安全地执行操作。它涵盖的主题包括变量的可见性、指令重排、原子操作等，旨在解决由于多线程并发编程带来的一些问题。
