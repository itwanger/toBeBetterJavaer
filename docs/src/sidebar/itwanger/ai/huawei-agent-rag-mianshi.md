...
【截图：沙箱双重隔离；风格：whiteboard；截图目标：展示只有文件隔离、只有网络隔离、两者都有三种情况下的攻击路径；关键词：沙箱、文件隔离、网络隔离】
...
【截图：BM25 只做重排的漏召回；风格：data-board；截图目标：展示关键词完全匹配但向量得分低的文档在向量召回阶段被刷掉；关键词：BM25、向量召回、漏召回】
...
【截图：上下文检索效果；风格：data-board；截图目标：展示 Contextual Retrieval 各组合下检索失败率的下降幅度；关键词：Contextual Retrieval、检索失败率、rerank】
...
【截图：评委模型与人工校准；风格：swimlane；截图目标：展示评委模型批量打分、人工抽样复核、计算一致率的流程；关键词：LLM-as-a-judge、人工校准、一致率】
...
【截图：蓝绿索引切换；风格：swimlane；截图目标：展示新建索引、全量重建、校验、别名原子切换、回滚的流程；关键词：蓝绿切换、索引别名、回滚】
...
【截图：5 MiB 分片下限；风格：data-board；截图目标：对比服务端 compose 合并和下载到磁盘再上传两种方式；关键词：S3 分片下限、compose、5 MiB】
