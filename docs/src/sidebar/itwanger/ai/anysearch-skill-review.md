...
最让我抓狂的是，那些真正有价值的信息——学术论文、代码仓库、金融数据、法律文书——传统的搜索引擎根本摸不到。用搜索引擎的行话来说，这些内容藏在"深网"里，大概占互联网高价值信息的 80%。

直到上个月，一个叫 AnySearch 的项目上线仅一周就冲上了 Skills.sh 热榜 TOP1，截至当前，它在 GitHub 上的 Star 数量也来到了1.9K。就冲着这个上升势头，二哥近日也把这个 Skill 给装上了，深度体验了一把，并详细的研究了一下它的设计思路。
...
【此处插入AnySearch 架构示意图：截图目标：展示 AnySearch 从查询意图识别到结构化输出的完整流程；关键词：意图识别、数据源路由、深网穿透、结构化输出；建议位置：架构图】
...
不是"搜到了一些相关信息"，而是"直接拿到了一份可用的分析报告"。这两者的差距，就像你让实习生帮你查资料，一个给你丢了一堆网页链接让你自己看，另一个帮你把要点提炼好了还做了对比表格。

更让我觉得舒服的是搜索过程。传统搜索里 Agent 的思考过程特别冗长——搜一次发现不对，换个关键词再搜，抓到网页发现内容不够，再换一个。来回折腾下来，token 烧了不少，结果还不一定好。而 AnySearch 因为有意图识别和自动路由，Agent 只需要发起一次查询，剩下的由 AnySearch 在服务端完成。
...
代码搜索，我让它搜 "Claude Code MCP server 实现 GitHub Python"，指定 `domain: code`。搜出来的不是随机的网页链接，而是按项目结构组织的仓库列表，每个都有明确的技术栈、依赖和集成方式，跟直接在 GitHub 上搜的效果相当。但 Agent 可以更进一步，直接用这些结构化数据做后续处理，比如筛选出最近三个月还在活跃更新的、Python 写的、跟 MCP 相关的项目。

【此处插入AnySearch 代码搜索截图：截图目标：展示垂直领域代码搜索的返回结果格式；关键词：代码搜索、GitHub、结构化结果、domain code；建议位置：网页】

学术搜索，我搜了 "transformer attention mechanism optimization 2025 2026"，指定 `domain: academic`。返回的是来自 arXiv 和学术数据库的论文列表，有标题、作者、摘要、发布日期的结构化信息。以前做技术调研时，用户往往需要自己去 arXiv 一篇篇翻，现在 Agent 就可以直接帮我们完成初筛。不过坦白说学术搜索的覆盖面目前还不够广，有些比较新的预印本搜不到，估计是索引更新有延迟。

【此处插入AnySearch 学术搜索截图：截图目标：展示学术论文搜索的返回结果；关键词：学术搜索、arXiv、论文检索、domain academic；建议位置：网页】

金融搜索，我搜了 "NVIDIA 2026 Q1 revenue data center growth"，指定 `domain: finance`。返回了英伟达最新的季度营收数据，包括数据中心业务增长率、毛利率变化、和去年同期对比。数据也精确到了具体数字，而不是模糊的"大幅增长"。但如果要做专业级的金融分析，任何一个通用搜索产品都替代不了 Bloomberg Terminal，AnySearch 更适合做快速参考。
...
【此处插入AnySearch 批量搜索截图：截图目标：展示 batch_search 并行搜索多个主题的效率和结果；关键词：batch_search、并行搜索、横向对比、多主题调研；建议位置：网页】
...
【此处插入AnySearch 与传统搜索对比架构图：截图目标：展示两种搜索方案的链路差异；关键词：传统搜索、AnySearch、Agent 搜索、架构对比；建议位置：架构图】
...
【此处插入AnySearch 产品总结截图：截图目标：展示 AnySearch 的整体产品界面和使用效果；关键词：AnySearch、AI 搜索基础设施、Agent 搜索、深网；建议位置：网页】

缺点也有。垂直领域的深度还不够均衡，学术和金融领域的数据覆盖面有待提升。偶尔会出现索引延迟，新发布的内容搜不到。但考虑到它才上线不到一个月，这些问题都可以理解。

1.9K Star 不是白给的。如果你也在用 Claude Code、Codex 或者 PaiCLI 做开发，我强烈建议装上试试。装之前你可能会觉得"Agent 搜搜网页不就够了"，装之后你会发现——原来 Agent 能搜到的东西比你想象的多得多。
