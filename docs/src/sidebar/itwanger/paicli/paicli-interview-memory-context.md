...
如果面试官继续问“这是否等同于成熟产品的上下文管理”，应直接回答：**不等同**。PaiCLI 已有结构化摘要、分段覆盖、可选的提前生成会话摘要，以及超大工具结果落盘后的按需读回；旧对话中的普通工具结果目前没有单独的 microcompact。摘要仍可能遗漏细节，原始会话账本供审计，不能把它说成模型能自动找回的记忆。当前压缩也没有按主题确定性地锁定用户意图、文件和附件，更不能保证服务端 prompt 缓存一定命中。
...
第二种是摘要法，就是 PaiCLI 用的这种。用模型对早期对话生成摘要，用摘要替代原文。摘要是有损的，而且较长历史可能需要多次模型调用。
...
**项目简介**：从零实现的终端 AI Agent，以实际发送给模型的会话历史承载当前上下文，用本地 JSON 保存明确记录的长期事实；代码可走精确搜索或按需语义检索，并按模型窗口调整上下文预算。

**技术栈**：Java 17、JavaParser AST、Ollama nomic-embed-text Embedding、SQLite、分段摘要、NIO FileVisitor

**核心职责**：

1. 管理 Agent 实际使用的会话历史，并将明确保存的长期事实持久化到 JSON 文件，支持项目和全局作用域
...
4. 在每轮 LLM 调用前检查真实消息历史，按 user 边界保留最近 3 个用户轮次；旧历史过长时分段摘要并按顺序合并，可选的提前摘要路径失败时回退完整摘要
