...
老王翻了翻我的简历，“你这个 PaiCLI 写了会话上下文、长期记忆、代码检索和长上下文管理，具体怎么做的？”
...
我说：“可以从会话上下文、长期事实和按需检索三个方面理解，但这不是三份都会自动写入模型的独立记忆。”

当前会话真正发给模型的是各 Agent 的 `conversationHistory`，其中有用户输入、模型回复、工具调用和结果。另有原始会话账本持久化这些消息，供审计和排查；账本不会自动替代模型当前的消息列表。

长期记忆是跨会话持久的。

用户说“记一下这个项目用 Java 17”，Agent 就把这条事实写到本地 JSON 文件里。之后每个用户轮次开始前，Agent 都按本轮输入从文件里检索相关条目，注入到 system prompt 里。这样跨会话 Agent 也能“记住”用户的偏好和项目背景。

长期记忆旁边还有一层 PAI.md 项目记忆，放团队共享的项目规则。PaiCLI 每次构建 system prompt 都会按顺序读取用户目录、项目根目录和 `.paicli/` 下的 PAI.md 与 PAI.local.md，总长度不超过 24000 个字符。它和长期记忆的分工是，团队都该遵守的规则写进 PAI.md，个人偏好和会变化的项目事实交给长期记忆。
...
我说：“要分清自动注入和按需检索。`conversationHistory` 是模型实际使用的会话消息；长期记忆可以按项目作用域检索并注入相关内容；代码搜索或向量检索通常由工具按需执行。不能把它们描述成每轮都自动检索、统一拼装的三层系统。”
...
PaiCLI 在发起模型请求前估算当前对话的 Token 占用，达到 `ContextProfile.compressionTriggerTokens()` 时尝试压缩。阈值按窗口预留摘要输出空间和缓冲，并非统一的 90%；估算值也不是精确计数。

老王追问：“压缩具体怎么做？”

![](https://cdn.paicoding.com/stutymore/build-agent-p3-memory-20260420221555.png)

我说：“压缩作用于 Agent 真正发给 LLM 的 `conversationHistory`。默认走完整摘要：保留 system 和最近 3 个 user 消息开始的尾部，从 user 边界切出更早消息。旧消息在 60000 字符以内时做一次摘要；超过时按顺序分段摘要，再合并成四栏目记录。最终摘要缺少规定栏目会尝试重整一次，仍不合格就不替换原历史。”

“另有默认关闭的实验性 Session Memory 快速路径，可以提前生成摘要；不可用时回退到完整摘要。它不是一份独立注入模型的短期记忆，也不能说两个互不相关的压缩器会分别压两份上下文。”
...
摘要之前其实还有两档更便宜的处理。单个工具结果超过 32000 个字符，完整内容先落盘到项目的 `.paicli/tool-outputs/` 下，上下文只留路径和首尾预览；对话越过清理阈值（默认取 100000 和摘要阈值 60% 中的较小值）时，较早的工具结果正文被换成一句占位说明，`tool_call_id` 保持不变，最近 3 条不动。清理完如果已经低于摘要阈值，这一轮就不做摘要。

手动输入 `/compact` 走的是同一套完整摘要，只保留最近 1 个 user 轮次。摘要重建后估算 Token 没有下降，也会放弃压缩，保留原历史。

如果面试官继续问“这是否等同于成熟产品的上下文管理”，应直接回答：**不等同**。摘要仍可能遗漏细节，原始会话账本只供审计，不能把它说成模型能自动找回的记忆。压缩判断用的是字符估算，没有计入工具 schema 和思考模型回传的 reasoning 内容，估算偏小时会压得偏晚。当前压缩也没有按主题确定性地锁定用户意图、文件和附件，更不能保证服务端 prompt 缓存一定命中。
...
第三条是 9 月加的自动提取。顶层任务正常完成后，程序从用户这一轮提交的原文里挑出最多 3 条稳定偏好或项目事实，挑出来的片段必须在原文里逐字出现。自动条目只存 project 作用域，标“待核实”；遇到重复或冲突一律跳过。会话里读过网页、浏览器或 MCP 返回的内容之后，自动提取会暂停，防止外部内容被当成用户偏好写进去。
...
我说：“每轮对话开始前，检索长期记忆的时候多了一层项目可见性过滤。global 级的记忆对所有项目可见，project 级的记忆只对元数据里的项目路径匹配当前项目的会话可见。过滤完可见性之后，才进入关键词匹配和评分排序，最多取 10 条，再按 Token 上限截断。”

Token 上限随窗口变化，公式是 `max(500, min(5000, window / 200))`，200K 窗口是 1000，1M 窗口是 5000。ReAct 模式把结果放进 system prompt 的 Project Context 段；Plan 模式目前是追加在每个任务的 user 输入末尾，两种模式的注入位置还没统一。

检索用的是最朴素的关键词匹配，不是向量检索。

因为长期记忆通常就几十条，关键词匹配够用且零依赖。如果记忆量到了几千条，就应该换成向量检索了。

老王追问：“记错了怎么办？比如项目早就升到 Java 21，记忆里还是 17。”

我说：“这是长期记忆最麻烦的地方，PaiCLI 从写入和使用两头防。”

写入时，`/save` 和 `save_memory` 会做冲突检测。同一作用域里两条内容只有数字或版本号不同，或者字符二元组相似度达到 0.8，新内容就不写入，把新旧两条都列给用户，由用户选择 `/memory replace` 替换、`/save --force` 两条都留，或者保持不变。模型不能替用户做这个选择。

使用时，每条记忆都带写入日期和最后核实日期，超过 30 天没核实的标“可能已过时”。注入内容开头还有一句提醒，记忆是线索不是事实，涉及版本、配置、路径的信息，行动前先对照当前文件核实。

管理命令也跟着补齐了：`/memory list` 查看所有记忆，`/memory search` 按当前项目可见性搜索，`/memory delete` 删除单条，`/memory verify` 刷新核实时间，`/memory replace` 替换内容。让用户能看到 Agent 到底记住了什么，心里有底。
...
| 参数 | 规则 |
|---|---|
| 压缩触发线 | `window - min(20000, max(1000, window / 4)) - min(13000, max(1000, window / 8))`，并限制在有效窗口内 |
| 会话上下文 | Agent 的 `conversationHistory`；当前没有单独的 45% 短期记忆预算 |
...
对话历史是追加式的，新一轮请求等于旧请求加上新消息，大部分前缀都重复。同一个用户轮次里的多次工具调用，system prompt 和前面的历史完全不变，缓存命中率最高。

PaiCLI 组装 prompt 的时候遵循“越稳定的内容越靠前”的原则：系统提示词、人格设定、模式指令这些稳定内容放前面，项目上下文、技能索引、记忆等动态内容放后面。这样更容易让 provider 的前缀缓存命中。

有两个地方会让缓存失效，面试时最好主动说出来。ReAct 每个用户轮次都按本轮输入重新检索长期记忆，重建整个 system prompt，system 里的记忆段一变，从这里往后的缓存就失效了。旧工具结果清理和摘要压缩也会改写历史中间的消息，缓存会在改写点再失效一次。更好的做法是让 system 只放稳定内容，本轮检索到的记忆附在本轮 user 消息后面，这是 PaiCLI 下一步要改的地方。

DeepSeek 走 automatic prefix cache；GLM、Kimi、Step、混元也在 `LlmClient` 里声明了对应的 prompt cache mode。PaiCLI 会从 `cached_tokens`、`prompt_cache_hit_tokens`、`input_cache_hit_tokens` 等字段里解析缓存命中量，并在状态栏展示。

### 11、上下文压缩有哪些策略

老王问：“除了你们用的摘要法，还有其他压缩策略吗？”

我说：“主流的有三种。”

第一种是截断法，最简单粗暴，直接丢弃最早的对话。

第二种是摘要法。用模型对早期对话生成摘要，用摘要替代原文。摘要是有损的，而且较长历史可能需要多次模型调用。

第三种是选择性保留。保留 system prompt 和最近几轮，把占空间最大、最容易重新获取的内容先清掉。Anthropic 的 context editing 提供的 `clear_tool_uses` 就是这个思路，清理的是旧工具结果。

PaiCLI 现在三种手段按代价从小到大组合使用。超大工具结果先落盘，上下文只留路径和预览，原文随时能读回；越过清理阈值后，较早的工具结果换成占位说明，这一档参考的就是 `clear_tool_uses`，默认也保留最近 3 次；仍然超过摘要阈值才做有损的摘要。截断法没有单独使用，完整摘要的分段合并保证了中段对话不会被静默丢掉。

老王又问：“压缩的时机怎么选？”

PaiCLI 当前要看的是 **conversationHistory 压缩**，不能再按一份独立的短期记忆预算来算触发点。

这是 Agent 真正要发给 LLM 的消息列表，防止上下文窗口爆掉。它的触发时机是在 **每次调用 LLM 之前**，不是任务结束后，也不是报错后才压缩。

ReAct 主循环、Plan 每个 task 的执行循环、Multi-Agent 的 SubAgent 循环，都在发起下一轮 LLM 请求前检查一次。如果当前 conversationHistory 估算 Token 达到 `compressionTriggerTokens()`，就尝试压缩。可选的 Session Memory 快速路径提前准备摘要，失败则走完整摘要。

所以按模型算，conversationHistory 的压缩阈值大概是：

- 1,000,000 token 窗口：约 `1,000,000 - 20,000 - 13,000 = 967,000 tokens`
- 200,000 token 窗口：约 `200,000 - 20,000 - 13,000 = 167,000 tokens`
- 256,000 token 窗口：约 `256,000 - 20,000 - 13,000 = 223,000 tokens`
- 128,000 token 窗口：小窗口按比例缩小预留，约 `128,000 - 20,000 - 13,000 = 95,000 tokens`

旧工具结果清理的阈值更低，200K、256K、1M 窗口都是 100,000，128K 窗口约 57,000。

压缩方式按 `user message` 边界切割，保留最近 3 个用户轮次。更早的消息先按最多 60000 字符的连续片段覆盖摘要，必要时分层合并；最终校验四个栏目后才重建成：
...
一句话概括就是：**PaiCLI 在每轮 LLM 请求前主动检查估算占用，阈值随窗口变化；压缩结果仍需警惕摘要遗漏和估算误差。**
...
我说：“直接报错或者模型理解混乱。tool 消息没有匹配的 tool_call_id？API 返回 400。assistant 和 user 顺序搞乱了？模型分不清谁说了什么。把工具结果塞进 user 消息？模型会把网页或文件里的内容当成用户本人说的话，里面夹带的指令就可能被执行。”

PaiCLI 的做法是工具结果一律走 tool 消息，而且进入历史之前都包在 `<tool_result tool="..." trust="untrusted-data">` 标签里，内容里伪造的同名标签会被转义。system prompt 声明标签内只是数据，里面的指令一律不执行。

我说：“做摘要压缩的时候有个特别容易踩的坑。被压缩区里如果有工具调用，只摘掉 tool 结果、留下带 tool_calls 的 assistant 消息，API 就会发现有个 tool_call_id 找不到对应的 tool 结果，直接报错。”

PaiCLI 的处理是切点只落在 user 消息上，被压缩区里的 assistant 工具调用和 tool 结果整体进入摘要，一条不留。重建后是一条装摘要的 user 消息，加一条不带 tool_calls 的 assistant 确认，后面接完整的尾部。旧工具结果清理那一档则只替换 tool 消息的正文，消息条数和 tool_call_id 都不变，配对关系天然保留。
...
**项目简介**：从零实现的终端 AI Agent，以实际发送给模型的会话历史承载当前上下文，按落盘、清理旧工具结果、摘要三档控制长度；长期事实保存在本地 JSON，支持作用域、冲突检测和自动提取；代码可走精确搜索或按需语义检索。

**技术栈**：Java 17、JavaParser AST、Ollama nomic-embed-text Embedding、SQLite、分段摘要、NIO FileVisitor

**核心职责**：

1. 设计长期记忆写入流程，支持项目和全局作用域、数字版本差异与相似度冲突检测、30 天未核实过时标注，并实现只从用户原文逐字抽取的自动事实提取
...
4. 实现三档上下文压缩，超大工具结果落盘可读回，旧工具结果替换为占位说明且保持 tool_call 配对，最后按 user 边界分段生成四栏目结构化摘要，摘要不合格或压缩无收益时保留原历史
