...
def main():
    ...
    # WORKSPACE resolves to the repo's docs/src/ai/script (this shared/ lives in-repo).
    ...
    # relpath raises on Windows when source and project sit on different drives; record the absolute path then.
    try:source_ref=os.path.relpath(source,project)
    except ValueError:source_ref=str(source)
    write_json(project/'project.json',{'schemaVersion':1,'slug':slug,'title':title,'compositionId':composition,'outputName':slug+'.mp4','sourceArticle':source_ref,'config':cfg,'audioUnitMode':'meaning-groups'})
