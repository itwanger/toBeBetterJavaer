...
【截图：短期记忆 vs 长期记忆；风格：skill-card；截图目标：左右对比，左边"短期记忆"标注内存、当前对话、关掉就没，右边"长期记忆"标注磁盘、文件持久化、自动加载；关键词：内存、磁盘、对话记录、持久化文件】
...
【截图：长期记忆四步流程；风格：swimlane；截图目标：从左到右四个步骤依次展开——提取（从对话识别关键信息）→ 存储（写入磁盘文件）→ 检索（按相关性匹配）→ 注入（塞进系统指令）；关键词：提取、存储、检索、注入】
...
【截图：记忆反思三件事；风格：checklist-card；截图目标：三行卡片式列举——① 过时的更新（换工作→旧职位标记过期）② 重复的合并（多次"偏好 Python"→合成一条）③ 修正矛盾（素食→吃牛排→主动跟进）；关键词：更新、合并、修正】
