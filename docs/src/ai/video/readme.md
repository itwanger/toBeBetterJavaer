...
完整答案：[查看图文解析](./what-is-agent-checkpoint.md)

🟡 进阶 | `→ Claude Code / Codex / PaiCLI`

### 19. Agent 的沙箱（Sandbox）你了解吗？

<p class="agent-new-question"><span>NEW</span> 新增题目</p>

命令黑名单在动态生成的代码与复杂的系统调用面前形同虚设，合格的沙箱绝不是在命令表面贴创可贴，而是必须在系统底层筑牢文件系统读写、网络出入站、进程空间与硬件资源配额四道防线。一线工程在隔离强度和启动成本之间各有取舍：Claude Code 采用 Git Worktree 隔离工作区并结合抽象语法树（AST）解析命令，Codex 在操作系统层依托 Bubblewrap 命名空间与 Seatbelt 规则锁死未授权读写与网络，Devin 则在云端直接拉起具备硬件级隔离的独立微型虚拟机。对于轻量本地 Agent，通常通过前置物理路径围栏、危险指令快速熔断、人机协同（HITL）审批以及系统原生沙箱包装来逐层设防。

完整答案：[查看图文解析](./what-is-agent-sandbox.md)
