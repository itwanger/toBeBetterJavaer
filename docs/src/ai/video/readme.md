...
### 17. 什么是 Agent 的观察空间和动作空间？

<p class="agent-new-question"><span>NEW</span> 新增题目</p>

观察空间决定模型能看到什么，动作空间决定模型能做什么。没有进入上下文的信息，对模型来说等于不存在；没有被工具允许的操作，模型只能停留在文字建议上。底层模型固定时，提升任务表现最主要的手段就是扩展这两个空间，也就是扩展上下文和工具。很多看起来需要更强模型的问题，其实是接口问题。
