...
本机为 Windows 11，按以下差异执行，其余以上游 WORKFLOW.md 为准：

- 用 `python`（本机无 `python3`）；Shell 为 PowerShell，`&&` 不可用，用 `;` 或 `if ($?) { }`。
- 首次使用或依赖变更后，在 `docs/src/ai/script/` 下执行 `npm ci` 安装共享依赖；每次 `npm ci` / `npm install` 之后必须重跑 `node docs/src/ai/script/shared/tools/patch_windows.mjs`（幂等），否则 `still` / `render` 报 `EPERM: operation not permitted, symlink`。
- `remotion/public` 下的链接在本机是 junction；跨盘或跨目录移动项目后链接会失效，需先 `cmd /c rmdir` 拆除再调用 `link_public` 重建。
- ffmpeg 位于 `docs/src/ai/script/.tools/ffmpeg/`，已加入用户 PATH；render 的音轨封装依赖它。
- `remotion.mjs` 已内置 Windows Chrome 探测；Python 工具读写中文文件均已显式 `encoding='utf-8'`，改工具时不要回退这些补丁。
...
- `remotion/public/images`、`remotion/public/audio` 是指向本项目 `assets/images`、`build` 的链接（macOS 为相对软链接，本机 Windows 自动降级为 junction），由初始化工具创建。组件继续用 `staticFile('images/...')` 和 `staticFile('audio/voiceover.wav')`；不要重复拷贝媒体。
