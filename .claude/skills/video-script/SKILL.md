...
用 `AskUserQuestion` 确认不清楚的信息。主题的边界、有没有必须覆盖的知识点。热点稿实测型还要问用户有没有一手实测材料，截图、记录、心得。
...
四篇标杆稿件，各有一个差异化亮点。步骤 1 确认主题后，用 `AskUserQuestion` 让用户选择参照哪篇的风格来写，把该篇的亮点融入新稿。
...
- 步骤 1 确认主题和类型后，展示上面四个选项让用户选
- 用户选定后，读取对应标杆文件，提取该篇的结构特征和亮点写法
- 撰写时把该篇的差异化亮点融入新稿。比如选了"硬核+开源推荐"，正文讲完后要加 4-5 个相关 GitHub 项目的推荐段；选了"突发事件解读"，结尾要给可操作的验证方法
