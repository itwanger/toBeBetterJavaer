...
| 深度拆解类 | 深层：读源码、翻 PR、跑实验，每个核心观点都要有一手证据 |
...
- **安装教程类**
- **产品评测类**
- **面试八股类**：对话体，每道题直问直答（专属规范见 `references/interview-style.md`）。员工爆料、内部消息只是前言的开头素材，不是单独的文体
- **深度拆解类**：围绕一条问题链拆解一个机制，靠源码和实测推进（写法指南见 `references/deep-analysis.md`）
...
2. **结构骨架**：章节标题、每道题的题型判断和推进线。推进线用一句话串起来（起点问题 → 第一个手段 → 它留下的问题 → 下一个手段），不要分条列要点，大纲分条，正文就会跟着分条
