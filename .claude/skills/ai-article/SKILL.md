...
正文章节（`## 01` 到最后一个编号章节）保持书面语，不安排金句，不模仿金句的语气。

读 `human-tone.md` 是为了找"有判断、有画面感、有人情味"的感觉，不是为了把里面的口语词汇搬进正文。金句素材里的"tm/牛逼/。。"等原始口语，任何位置都不照搬。
...
每个章节（包括二级标题、三级标题、四级标题）至少 1 个占位符，章节长的话，可以安排3个占位符（前中后），格式：
