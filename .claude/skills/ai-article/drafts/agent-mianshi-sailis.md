...
辛辛苦苦用 Agent 做座舱测试，项目确实有产出，结果领导来一句“我们不看项目结果，只看积极性”，连续两次被打 C。

有一说一，这种事在不少公司都见过。技术人最怕的就是干完了才发现评价标准跟你的产出压根不在同一个维度上。

你拼的是结果，人家看的是态度分。

不过反过来想，能在赛力斯做座舱 Agent 相关的工作，说明这位同学手上的技术是有东西的。

说回赛力斯。

赛力斯的竞争力在于把华为的智能化、品牌和渠道势能，通过自己掌握的问界品牌资产、整车研发能力、超级工厂和供应链体系，稳定兑现成高端汽车销量。2024年赛力斯花了25亿从华为手中收购了问界全部919个商标，又投了115亿入股华为引望10%。

问界 M9 累计交付突破27万辆，50万级年度销冠。2025年赛力斯研发投入125亿，同比增长77.4%，研发人员9019人，占总员工41%。

【截图：赛力斯华为合作架构；风格：whiteboard；截图目标：展示赛力斯和华为在问界品牌上的分工关系；关键词：华为、赛力斯、问界、MoLA】

赛力斯披露，工业 AI 模型已经覆盖36000多个质量点位。AI 视觉检测站通过十多套高速工业相机，100秒内完成整车全方位扫描，一站式完成60多个检测项，日均纠偏超200次。

超级工厂里3000多台机器人在跑，关键工序100%自动化。

座舱智能化方面，华为在2026年4月发布了 MoLA 2.0（Mixture of Large Model Agent）架构，参数规模突破720亿。五大垂域 Agent 分别负责导航、车控、娱乐、服务和安全，由 System Agent 统筹决策，鸿蒙座舱用户已经突破120万。

从产品层面看，汽车已经具备了 Agent 的几个关键要素。摄像头和雷达做环境感知，座舱理解用户意图，导航引擎做路径规划，车辆控制系统相当于工具调用，传感器提供实时反馈，用户偏好就是长期记忆，OTA 让整个系统持续进化。

比通用的聊天 Agent 更接近“物理世界 Agent”。

销量、工厂和服务网络产生的真实场景数据，又持续为下一代产品和 AI Agent 的改进提供素材。

我去帮大家调研了华为 MoLA 架构、鸿蒙座舱、车规 OTA 这些方向的技术细节，整理成了11道面试题的完整解答。每道题背后都是一个真实的工程约束，跟着一起学，面试的时候能直接接住。

【截图：面试题预览；风格：skill-card；截图目标：展示11道座舱Agent面试题的主题分布；关键词：座舱Agent、端云切分、安全边界、Token预算】

（全文比较肝，保证大家能学到很多很多，系好安全带，我们粗粗发～）

## content

【截图：座舱Agent面试题概览；风格：skill-card；截图目标：11道题目全景图；关键词：座舱Agent、端云协同、鸿蒙生态、车规OTA】

### 01、座舱 Agent 应该放在车端还是云端？端云怎么切分？

“按实时性和安全性来切分。对响应延迟敏感的、跟行车安全相关的功能，必须放端侧。”
...
【截图：端云切分架构；风格：whiteboard；截图目标：展示端侧和云端的任务分布和数据流向；关键词：端侧、云端、MoLA、垂域Agent】
...
还有一个趋势值得关注，行车的时候算力优先保障智驾，驻车的时候释放给座舱做更复杂的交互，整体利用率能从不到30%提升到70%以上。

### 02、车机算力有限，端侧模型怎么裁剪才不影响意图识别准确率？

“量化压体积，蒸馏转能力，剪枝减冗余，三种手段组合用。”

第一，量化。把模型参数从 FP32 降到 INT8，体积直接压缩到四分之一。

如果用 QAT（Quantization-Aware Training，量化感知训练），在训练阶段就让模型适应低精度，精度损失比训练后量化小很多。

第二，知识蒸馏。用云端千亿参数的模型当老师，教端侧的小尺寸模型学。

蒸馏后推理速度能提升3倍以上，推理延迟从大几百毫秒压到150毫秒以内。

第三，结构化剪枝。在 channel 或 layer 维度移除贡献小的神经元，不破坏模型整体结构，部署的时候不依赖特定硬件。

“三种手段叠起来，模型体积压缩到原来的四分之一以下，保持95%以上的精度。”

【截图：模型压缩三步流程；风格：swimlane；截图目标：展示量化蒸馏剪枝的流程和效果；关键词：INT8量化、知识蒸馏、结构化剪枝】

精度能不能保住，取决于微调阶段用什么数据。云端模型见过的语料太杂，端侧需要用车载场景的专属数据做领域蒸馏。

比如说“打开空调”“导航回家”这些在车里出现频率最高的指令，蒸馏的时候重点保这些。线上运行之后持续收集 badcase 做增量微调，端侧模型的精度会越跑越高。
...
还有一个预缓存策略，信号好的时候预加载可能用到的资源，比如说导航已经规划好的路线数据、目的地附近的 POI 信息。进了隧道也能撑一阵。

【截图：断网降级策略；风格：three-layer；截图目标：展示功能分级降级的层次关系；关键词：端侧兜底、功能分级、预缓存、网络检测】
...
【截图：操作风险分级表；风格：checklist-card；截图目标：展示不同操作的风险等级和处理方式；关键词：安全边界、风险分级、多模态确认、SOTIF】
...
【截图：多模态感知数据流；风格：swimlane；截图目标：展示传感器到Agent的数据处理流程；关键词：RGB、红外、麦克风阵列、AMS】
...
【截图：隐私数据处理流程；风格：three-layer；截图目标：展示端侧处理和云端上报的边界；关键词：端侧处理、数据不出车、特征向量、个人信息保护】

个人信息保护法要求处理个人信息遵循“合法、正当、必要”原则，收集限于“最小范围”。车主享有个人信息遗忘权。
...
【截图：对话中断恢复流程；风格：swimlane；截图目标：展示中断检测、状态存档、恢复提示的完整流程；关键词：DST、状态存档、主动恢复、全双工】
...
“整个语音交互流程拆成五个环节，从用户开口到 Agent 开始出声，目标是 p50 控制在400毫秒以内。”
...
【截图：延迟预算拆解图；风格：swimlane；截图目标：展示五个环节的延迟分布和流式并行优化效果；关键词：VAD、ASR、LLM TTFT、TTS、流式并行】

“人类对话的自然响应间隙差不多200毫秒。400毫秒以内用户感觉流畅，超过1.5秒用户可能就重新说一遍或者直接放弃了。”

端侧推理也能帮忙。地平线征程6平台的座舱对话模型做到了语音交互延迟80毫秒以内。
...
【截图：鸿蒙跨设备调用架构；风格：whiteboard；截图目标：展示分布式软总线、超级终端和原子化服务的关系；关键词：分布式软总线、超级终端、FA、PA】
...
【截图：车规OTA灰度发布流程；风格：swimlane；截图目标：展示灰度推送、验证、回滚的完整流程；关键词：A/B分区、灰度发布、MiL/SiL/HiL、Prompt注册】

车规验证绕不开。2026年1月起国家强制性标准生效，OTA 升级必须通过 MiL（模型在环）、SiL（软件在环）、HiL（硬件在环）三级验证，覆盖极端温湿度、电磁干扰、网络中断等工况。
...
【截图：世界模型与座舱Agent对比；风格：data-board；截图目标：对比两套系统在安全等级、延迟、输出类型上的差异；关键词：世界模型、座舱Agent、ASIL D、计算隔离】
...
【截图：可共享组件和隔离边界；风格：whiteboard；截图目标：展示哪些组件可以共享、哪些必须隔离；关键词：意图共享、传感器共享、计算隔离】
...
【截图：Token预算优化策略；风格：data-board；截图目标：对比优化前后的Token消耗和成本；关键词：端侧推理、模型路由、语义缓存、KV缓存】
...
“API 定价的趋势也在帮忙。2025到2026年，大尺寸模型的 API 价格降了大概80%。”
...
【截图：简历核心职责结构；风格：skill-card；截图目标：展示简历包装的结构和要点；关键词：多Agent、端云协同、安全边界、灰度发布】
