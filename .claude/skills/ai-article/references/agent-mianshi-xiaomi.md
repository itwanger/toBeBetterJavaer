...
> 参考说明（2026-09-24）：学这篇的直问直答节奏和判断力。05、08、10 已改成因果推进的写法，答案怎么往前走以这三题为准。04、07、11 里的分层是题目涉及的真实架构分层（9 层 Prompt、Skills 三层加载、TUI 三层），可以保留，但不要把分层写法套到非分层的问题上。
...
“LLM 在长对话中容易忘了前面做过什么，重复操作，也容易漏掉某个步骤，直接跳到最后。to do list 把‘已完成’和‘待完成’的步骤显式列出来，模型每一轮都能看到当前进度，自然就知道下一步该做什么。”

“就像我们开发用看板一样，任务列在上面，做完一个划掉一个。模型也需要这种外部记忆辅助。”

### 05、查询改写做过吗？多维度查询改写是什么？

“做过。用户的原始提问往往不是最好的检索 query，查询改写就是把它改成更容易命中文档的样子。”

“最常见的问题是用词对不上。用户问‘Agent 性能’，文档里写的是‘延迟’‘吞吐量’‘响应时间’，所以先做同义扩写，用几种不同的说法各检索一次。”

【截图：三种查询改写策略】

“扩写解决了用词，解决不了问题本身太杂。‘比较 A 和 B 的性能和价格’这种复合问题，一条 query 带着四个关注点，召回的文档哪个都沾一点、哪个都不全，这时候要拆成几个单一问题分别检索。”

“拆完以后，有些问题在知识库里压根没有直接答案。比如问的是某个具体产品，文档里只有品类层面的横向对比，这时候往上泛化一层，把‘Claude Code’泛化成‘终端 Agent’，至少能召回同类产品的对比内容。”

“几条改写后的 query 并行检索，结果按文档 ID 去重，再取 top-K 合并。”
...
“先看任务有没有完成。但成功率要可信，评测环境必须能复位，每次都从同一个状态出发，上一次运行留下的副作用不能污染下一次。”

“只看成功率又不够。两个 Agent 都能完成任务，一个用 3 步，一个用 12 步，水平差距藏在过程里，所以还要记录完成同一个任务用了多少步、消耗了多少 token、工具调用成功率多少。”

【截图：三层评估指标体系】

“这些指标要能反复跑才有意义。PaiCLI 的代码搜索模块有一个 Golden Set，123 个确定性测试用例，每条用例定义输入 query 和预期命中的文件行号，跑一遍就知道搜索有没有回退。”

#### 没有用户反馈怎么抽检？

“没有用户反馈，就让模型当裁判（LLM-as-Judge），按评分标准给产出打分。成本低、速度快，能覆盖大批量样本。”

“模型裁判自己也会漂移，所以要定期抽一批它打过分的样本让人工复审，算一下模型裁判和人类判断的一致率。一致率低于阈值，说明评分标准要调整，或者模型裁判本身出了问题。”
...
“这个问题的根源是提示词之间存在隐式耦合。改了一条指令，模型对所有任务的行为都可能变化，因为提示词是全局生效的。”

“既然改一处会影响全局，改之前就得知道全局现在是什么样。所以先把现有能力覆盖的典型用例收集成评测集，改之前跑一遍，改之后跑一遍，对比分数。”

【截图：Prompt 调优回归测试流程】

“能发现问题还不够，还要控制改动的影响范围。提示词按关注点拆成独立文件，system 层、模式层、Skill 层各管各的，改 Skill 层的指令就不该影响 system 层的行为。”

“拆开以后，一次也只改一个地方。同时改两处出了问题，分不清是哪处改坏的。每次‘修好 A 坏了 B’，就把 B 加进回归用例池，下次改动时它会第一个报警。”
...
“Self-Attention 做的事情，是让每个 token 根据和其他 token 的相关程度，重新组合出自己的表示。”
