...
## 一、Prompt 组装（PromptAssembler.java）
...
3. **模式层**：根据执行路径加载不同指令集（ReAct / Plan / Team 各一套），通过 PromptMode 枚举切换
...
7. **Skills 索引**：通过 SkillIndexFormatter 格式化，上限 20 个 Skill / 4KB
8. **上下文管理策略**：告诉模型什么时候压缩、怎么压缩
9. **收尾指令**（handoff.md）：会话结束引导

**设计要点**：静态内容排最前面，Prompt Caching 按最长公共前缀命中，前缀越稳定缓存命中率越高。

**长期记忆注入**：在 Agent.run() 中，每轮 LLM 调用前通过 memoryManager.buildContextForQuery() 检索相关记忆，更新到系统提示词中。

---

## 二、上下文压缩（两层实现）

### 第一层：短期记忆压缩（ContextCompressor.java）

- 压缩对象：ConversationMemory 条目
- 压缩方式：Map-Reduce 策略 + LLM 摘要
- 触发条件：shortTermMemory 超过预算时
- 保留最近 N 轮不压缩

### 第二层：对话历史压缩（ConversationHistoryCompactor.java）

- 压缩对象：实际发送给 LLM 的 List\<Message\>
- 触发时机：每轮 LLM 调用前检查（Agent.java maybeCompactHistory()）
...
## 三、工具调用流程（ToolRegistry.java + Agent.java）

### 三阶段流程

**阶段 1：LLM 生成 tool_call**
- ToolRegistry.getToolDefinitions() 生成工具 schema
- 11 个核心工具 + MCP 动态工具（mcp__{server}__{tool}）
- Schema 包含 name、description、parameters（JSON Schema）

**阶段 2：解析 + 审批**
- 从 response.toolCalls() 提取 ToolInvocation 列表
- 写操作（write_file、execute_command 等）走 HITL 审批流程
- PathGuard / CommandGuard 做安全策略检查
- 浏览器工具走 BrowserCheckResult 检查

**阶段 3：并行执行**
- 单工具直接执行
- 多工具并行：线程池，上限 MAX_PARALLEL_TOOLS = 4
- 批量超时 90 秒
- 执行结果按原始顺序返回
...
- shouldAudit 覆盖 write_file、execute_command、create_project、revert_turn、mcp__*
- AuditLog 记录 allow/denyByPolicy

---

## 四、Skills 体系（SkillRegistry.java + SkillContextBuffer.java）
...
| 生命周期 | 单次调用，结果回灌 | 加载后通过 SkillContextBuffer 注入，LRU 淘汰 |
| 存储位置 | 硬编码在 ToolRegistry | 文件：~/.paicli/skills/、.paicli/skills/、jar 内置 |
...
- load_skill 执行后，Skill 正文推入 SkillContextBuffer（LRU，最多 3 个）
- 下轮 user 消息发送前，SkillContextBuffer.drain() 将内容前置拼接到 user 消息
- drain 后清空缓冲区（一次性消费）
...
## 五、多 Agent 编排（AgentOrchestrator.java）
...
1. planner.run(userInput) → 返回 ExecutionPlan JSON
2. orchestrator 解析计划为 DAG（有向无环图）
3. 按拓扑排序分批执行：
   - 找出所有依赖已满足的任务
   - 分配给空闲 Worker 并行执行
4. 每批完成后 reviewer 检查质量
5. 不合格 → 带反馈重新执行（最多重试 2 次）
6. 全部完成 → 汇总结果

### 共享与隔离

**共享**：ToolRegistry（所有工具）、MemoryManager（长期记忆）、LlmClient（同一模型）
**隔离**：对话历史（各自独立）、SkillContextBuffer（各自独立）、系统提示词（角色专属，通过 PromptMode.TEAM_PLANNER/WORKER/REVIEWER 区分）
...
## 六、Plan 模式（Planner.java + ExecutionPlan.java + PlanExecuteAgent.java）

### 规划流程

1. 简单任务检测：isSimpleGoal() → 直接创建单任务计划
2. 复杂任务：LLM 生成 JSON 计划，包含 summary + tasks 数组
3. 每个 task 有 id、description、type、dependencies
4. 解析为 ExecutionPlan 对象，构建依赖图
...
ExecutionPlan:
  ...
  - getExecutableTasks(): 返回所有依赖已完成的任务

Task:
  - id, description, type (PLANNING/FILE_READ/FILE_WRITE/COMMAND/ANALYSIS/VERIFICATION)
  - status (PENDING/RUNNING/COMPLETED/FAILED/SKIPPED)
  - dependencies / dependents
  - isExecutable(): 检查所有依赖是否已完成
```

### 依赖图构建

- ExecutionPlan.computeExecutionOrder() 做 DFS 拓扑排序
- 检测环依赖，有环返回 false

### 增量修改机制

- 用户审查计划时可选择 SUPPLEMENT（补充需求）
- PlanReviewDecision.supplement(feedback) 传递反馈
- Planner.replan(plan, feedback) 基于原计划和反馈重新规划
- 重新走审查-执行流程
...
- 检索：MemoryRetriever 基于用户输入查询相关记忆
- 注入：每轮 LLM 调用前通过 memoryManager.buildContextForQuery() 注入系统提示词
- Token 预算：contextProfile.memoryContextTokens() 控制
