...
- 技术名词、缩写或多音字读法不确定、被用户指出异常时，先核对作者或官方一手来源，再用 `ttsText` 明确读法；搜索摘要和 AI 答案不能直接作为官方依据。ASR 识别出正确拼写不代表发音正确，需复听该词及前后衔接；无法试听时如实记录，不能标记读音验收通过。已核实的 SQLite 读法和局部修复流程见 [TTS 使用约定](references/VOLCENGINE_TTS_GUIDE.md#术语读音与局部修复)。
...
产品名称需要图标辅助识别时，先查共享品牌素材库，复用已核实来源的 Claude Code、Codex 等官方图标；同一排图标按视觉大小统一容器与文图间距，保留各自比例。新增并确认的品牌素材连同来源记录纳入共享库，供后续视频复用，具体清单见下方执行约定。
